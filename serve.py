...
@app.before_request
def before_request():
    # this will always request database connection, even if we dont end up
    # using it ;\
    g.db = connect_db()
...
def papers_similar(pid):
    ...
    # check if we have distances to this specific version of paper id
    # (includes version)
    if pid in sim_dict:
        ...
        if kok:
            # ok we have at least one different version of this paper, lets use
            # it instead
            id_use_instead = kok[0]
            return [db[strip_version(k)] for k in sim_dict[id_use_instead]]
        else:
            # return just the paper. we dont have similarities for it for some
            # reason
            return [db[rawpid]]
    ...
    if g.user:
        ...
        if recent_days is not None:
            ...
            out = [x for x in out if curtime -
                   x['time_updated'] < recent_days * 24 * 60 * 60]
...
def default_context(papers, **kws):
    ...
    ans = dict(papers=top_papers, numresults=len(
        papers), totpapers=len(db), render_format="normal")
    ans.update(kws)
    return ans
...
def rank(request_pid=None):
    ...
    return render_template('paper.html', **ctx)
...
def top():
    ...
    papers = [p for p in TOP_SORTED_PAPERS if curtime -
              p['time_updated'] < tt * 24 * 60 * 60]
    papers = papers_filter_version(papers, vstr)
    ctx = default_context(papers)
    return render_template('top.html', **ctx)
...
def library():
    ...
    ctx = default_context(papers, num_paper=len(ret))
...
if __name__ == "__main__":
    ...
    top_paper_counts = sorted(
        [(v, k) for k, v in top_counts.iteritems() if v > 0], reverse=True)
    ...
    tts = [time.mktime(dateutil.parser.parse(p['updated']).timetuple())
    ...
    if recompute_index:
        ...
        for pid in db:
            ...
            dict_authors = makedict(
                ' '.join(x['name'] for x in p['authors']), forceidf=5)
